...
class MasterAgent:
    def __init__(self):
        self._llm = None
        # We no longer call LocalModelHandler.get_llm() here.
        # It will be loaded lazily the first time self.llm is accessed or ainvoke is called.
        print("🤖 Master Agent: Initialized (Ready for lazy loading)")

    @property
    def llm(self):
        if self._llm is None:
            try:
                print("🤖 Master Agent: Lazy loading Local Model handler now...")
                from .local_llm_handler import LocalModelHandler
                self._llm = LocalModelHandler.get_llm()
                if self._llm:
                    print("🤖 Master Agent: Connected to Local Model")
                else:
                    print("⚠️ Master Agent: Local Model Handler returned None")
            except Exception as e:
                error_msg = f"❌ Master Agent: Error during lazy loading: {e}"
                print(error_msg)
                import tempfile, os as _os
                log_path = _os.path.join(tempfile.gettempdir(), "debug_master_init.log")
...
Write your response now:"""
        ...
        try:
            ...
            # DEBUG LOGGING — write to temp dir to avoid uvicorn reload triggers
            import tempfile as _tmp, os as _os
            _log_path = _os.path.join(_tmp.gettempdir(), "debug_master_response.log")
