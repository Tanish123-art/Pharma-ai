...
async def signup(user_data: UserCreate, user_service: UserService = Depends()):
    try:
        ...
        # If the email is invalid/bounces immediately, it will raise an error and abort signup
        try:
        ...
        # Create user with hashed password ONLY if email succeeds
        try:
...
async def login(
    ...
    print(f"🔑 [Auth] Login request received for: {form_data.username}")
    import time
    ...
    print(f"⏳ [Auth] Authentication took {time.time() - start_time:.4f}s")
