...
class UserService:
    async def get_user_by_email(self, email: str) -> Optional[dict]:
        try:
            # Check if database.users_collection is accessible (it might be None if connection failed)
            # We access it directly, if it raises AttributeError (NoneType has no attribute find_one), we catch it.
